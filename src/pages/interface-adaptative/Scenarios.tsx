...
function ToggleRow<T extends string>({
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-2">
      ...
      <div className="flex rounded-full border border-stone-300 dark:border-stone-700 p-0.5">
        {options.map((o) => (
          <button
            key={o.value}
            onClick={() => onChange(o.value)}
            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
              value === o.value ? 'bg-stone-900 text-white dark:bg-white dark:text-stone-900' : 'text-stone-500'
            }`}
          >
            {o.label}
          </button>
...
export function Scenarios() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-16">
      <p className="text-sm font-medium text-stone-500 dark:text-stone-400 mb-3">Interface adaptative / Scénarios</p>
      ...
      <section className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {drivingScenarios.map((s) => (
          <div key={s.id} className="rounded-2xl border border-stone-200 dark:border-stone-800 p-5">
            ...
            <p className="mt-2 text-xs font-medium uppercase tracking-wide text-stone-400">Priorité</p>
            <p className="text-sm text-stone-600 dark:text-stone-300">{s.priorityInfo.join(', ') || '—'}</p>
            {s.reducedInfo.length > 0 && (
              <>
                <p className="mt-2 text-xs font-medium uppercase tracking-wide text-stone-400">Réduit</p>
                <p className="text-sm text-stone-600 dark:text-stone-300">{s.reducedInfo.join(', ')}</p>
              </>
            )}
            <p className="mt-3 text-xs text-stone-500 italic">{s.driverControl}</p>
          </div>
        ))}
      </section>

      <section className="mt-16 rounded-2xl border-2 border-stone-900 dark:border-white p-6 sm:p-8">
        ...
        <div className="mt-8 grid sm:grid-cols-3 gap-4">
          <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 p-4">
            <p className="text-xs font-medium uppercase tracking-wide text-emerald-700 dark:text-emerald-400 mb-2">Priorité</p>
            <ul className="space-y-1 text-sm text-emerald-900 dark:text-emerald-200">
              {state.priority.map((i) => <li key={i}>{i}</li>)}
            </ul>
          </div>
          <div className="rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 p-4">
            <p className="text-xs font-medium uppercase tracking-wide text-amber-700 dark:text-amber-400 mb-2">Secondaire</p>
          ...
          <div className="rounded-xl bg-stone-100 dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-4">
            <p className="text-xs font-medium uppercase tracking-wide text-stone-500 mb-2">Masqué</p>
        ...
        <div className="mt-6 rounded-xl border border-stone-200 dark:border-stone-800 p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-stone-400 mb-2">Pourquoi l’interface s’adapte ?</p>
      ...
      <section className="mt-16">
        ...
        <div className="space-y-6">
          {storyboards.map((sb) => (
            <article key={sb.id} className="rounded-2xl border border-stone-200 dark:border-stone-800 p-6 sm:p-8">
              <h3 className="font-semibold text-lg mb-4">{sb.title}</h3>
              <dl className="grid sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                {storyboardFields.map((f) => (
                  <div key={f.key}>
                    <dt className="font-medium text-stone-500 dark:text-stone-400 mb-1">{f.stepLabel}</dt>
                    <dd className="text-stone-700 dark:text-stone-300">{sb[f.key]}</dd>
                  </div>
                ))}
              </dl>
              <div className="mt-4 pt-4 border-t border-dashed border-stone-300 dark:border-stone-700 grid sm:grid-cols-2 gap-x-8 gap-y-2 text-sm">
                <p><span className="font-medium text-emerald-700 dark:text-emerald-400">Bénéfice UX :</span> {sb.uxBenefit}</p>
                <p><span className="font-medium text-rose-700 dark:text-rose-400">Risque potentiel :</span> {sb.risk}</p>
              </div>
            </article>
          ))}
        </div>
