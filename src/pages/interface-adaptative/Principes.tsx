...
export function Principes() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-16">
      <p className="text-sm font-medium text-stone-500 dark:text-stone-400 mb-3">Interface adaptative / Principes</p>
      ...
      <div className="mt-10 space-y-5">
        {designPrinciples.map((p) => (
          <article key={p.id} className="rounded-2xl border border-stone-200 dark:border-stone-800 p-6 sm:p-8">
            <div className="flex items-center gap-3">
              <span className="shrink-0 h-8 w-8 rounded-full bg-stone-900 text-white dark:bg-white dark:text-stone-900 text-sm font-medium flex items-center justify-center">
                {p.order}
              </span>
            ...
            <ul className="mt-4 grid sm:grid-cols-2 gap-x-8 gap-y-1.5">
              {p.points.map((pt) => (
                <li key={pt} className="flex items-start gap-2 text-sm text-stone-700 dark:text-stone-300">
