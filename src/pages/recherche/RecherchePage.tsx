...
export function RecherchePage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-16">
      <p className="text-sm font-medium text-muted mb-3">Plateforme de recherche</p>
      ...
      <blockquote className="mt-10 border-l-4 border-paper pl-5 py-1 text-lg sm:text-xl font-medium max-w-3xl">
        « {centralStatement} »
      </blockquote>
      ...
      <section className="mt-14" aria-label="Démarche de recherche interactive">
          ...
          {researchPhases.map((phase, i) => (
            <button
              ...
              className={`flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-medium transition-colors ${
                active === phase.id
                  ? 'bg-paper text-ink border-paper border-paper'
                  : 'border-line-2 text-dim hover:border-line-2'
              }`}
            >
        ...
        <div className="mt-6 flex items-center gap-2 text-xs text-muted" aria-hidden="true">
          {researchPhases.map((p, i) => (
            <span key={p.id} className="flex items-center gap-2">
              <span className={i <= researchPhases.findIndex((x) => x.id === active) ? 'text-paper' : ''}>
                {p.name}
              </span>
        ...
        <div className="mt-8 rounded-2xl border border-line p-6 sm:p-8">
          ...
          <p className="mt-2 text-sm text-dim italic">{activePhase.question}</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {subPages.map((p) => (
            <Link
              key={p.to}
              to={p.to}
              className="rounded-xl border border-line p-5 hover:border-line-2 transition-colors"
            >
