...
export function CadreTheorique() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-16">
      ...
      <ResearchOnly>
        <section className="mt-10" aria-label="Glossaire des concepts mobilisés">
          <h2 className="text-lg font-semibold mb-4">Concepts mobilisés</h2>
          <div className="flex flex-wrap gap-2">
            {theoreticalConcepts.map((c) => (
              <span
                key={c.id}
                className="rounded-full border border-stone-200 dark:border-stone-800 px-3 py-1 text-sm text-stone-600 dark:text-stone-300"
              >
                {c.term}
              </span>
            ))}
      ...
      <section className="mt-14">
        ...
        <p className="text-sm text-stone-500 dark:text-stone-400 mb-6">Comparaison visuelle des trois notions.</p>
          ...
          {simplicityComparison.map((col) => (
            ...
            >
              ...
              <ul className="space-y-1.5">
                {col.points.map((p) => (
                  <li key={p} className="text-sm text-stone-600 dark:text-stone-300 flex gap-2">
        ...
        <blockquote className="mt-8 border-l-4 border-stone-900 dark:border-white pl-5 py-1 text-base sm:text-lg font-medium">
          « {appliedDefinition} »
        </blockquote>
      ...
      <section className="mt-14">
        ...
        <div className="rounded-2xl border border-stone-200 dark:border-stone-800 p-6 sm:p-8">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-3">
            {perceptionActionChain.map((step, i) => (
              <span key={step} className="flex items-center gap-2">
                <span className="rounded-lg bg-stone-100 dark:bg-stone-800 px-3 py-1.5 text-sm font-medium">
                  {step}
                </span>
          ...
          <div className="mt-6 pt-6 border-t border-dashed border-stone-300 dark:border-stone-700">
            ...
            <ul className="mt-2 flex flex-wrap gap-2">
              {cognitiveLoadImpacts.map((impact) => (
                <li
                  key={impact}
                  className="rounded-full border border-stone-200 dark:border-stone-800 px-3 py-1 text-xs text-stone-600 dark:text-stone-300"
                >
                  {impact}
                </li>
              ))}
      ...
      <section className="mt-14 rounded-2xl border border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-6 sm:p-8">
        <h2 className="text-lg font-semibold">{visualVsUsageSimplicity.title}</h2>
        <p className="mt-2 text-sm text-stone-700 dark:text-stone-300">{visualVsUsageSimplicity.explanation}</p>
      </section>
    </div>
  )
}
