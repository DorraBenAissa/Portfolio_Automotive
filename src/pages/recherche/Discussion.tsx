...
export function Discussion() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-16">
      ...
      <p className="text-sm font-medium text-muted mb-3">Recherche / Discussion</p>
      ...
      <section id="matrice" className="mt-10" aria-label="Matrice de confrontation">
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-line p-5">
          {confrontationSteps.map((s, i) => (
            <span key={s} className="flex items-center gap-2">
              <span className="rounded-lg bg-surface px-3 py-1.5 text-sm font-medium">{s}</span>
              {i < confrontationSteps.length - 1 && <span className="text-muted" aria-hidden="true">→</span>}
            </span>
          ))}
        </div>
      </section>
      ...
      <section id="conclusions" className="mt-14">
        ...
        <ol className="space-y-4">
          {discussionConclusions.map((c, i) => (
            <li key={c} className="flex gap-4 rounded-xl border border-line p-4">
              <span className="shrink-0 h-7 w-7 rounded-full bg-paper text-ink text-xs font-medium flex items-center justify-center">
                {i + 1}
              </span>
              <p className="text-sm text-dim">{c}</p>
            </li>
          ))}
        </ol>
      </section>
